...
* Структура таблиц БД была спроектирована отличным образом от моделей приложения. Я спроектировал её таким образом, чтобы у сущностей не было избыточной зависимости друг от друга, а была зависимость только через FOREIGN KEYS. Так же было разрешено отношение между таблицами ```pull_requests``` и ```users``` при помощи таблицы ```pr_reviewers```. 
...
## Ограничения
* В методе ```TeamCreate``` проверка существования пользовотелей из передаваемого списка в БД осуществляется за счёт итерации по списку пользователей и вызова метода ```FindUserById``` репозитория User. В избежании многократного обращения к БД можно создать метод, который будет принимать список пользователей и проверять наличие пользователей по всем ```user_id``` из списка. Также в начальном варианте при использовании итерации можно добавить обработку каждого запроса в своей горутине в целях оптимизации обращения к БД. Данные идеи не были реализованы в связи с недостатком времени.
